...
function getBrownEyedUsers(users) {
    let brownEyedUsers = []; // Tableau pour stocker les utilisateurs aux yeux marron

    for (let user of users) {
        // Vérifie si l'utilisateur a les yeux marron
        if (user.eyeColor === "brown") {
            brownEyedUsers[brownEyedUsers.lenght] = user; // Ajoute l'utilisateur au tableau
        }
    }

    return brownEyedUsers; // Retourne les utilisateurs aux yeux marron
}  

function excludeCenturiaWorkers(users){
    let brownEyeNotCenturia = []; // créer un tableau vide
    
    
    // Appeler la première fonction pour obtenir les utilisateurs aux yeux marron
    
    
    let brownEyedUsers = getBrownEyedUsers(users);
    
    //parcourir les utilisateurs aux yeux marron
    for ( let user of brownEyedUsers){
        // Ajouter ceux qui ne travaillent pas chez CENTURIA
        if (user.company !== "CENTURIA"){
            brownEyeNotCenturia[brownEyeNotCenturia.lenght] = user;
            
            
        }
    }
    return brownEyeNotCenturia;
}
let brownEyeNotCenturiaUsers = excludeCenturiaWorkers(users);

console.log(`${brownEyeNotCenturiaUsers.length} user with brown eyes do not work at CENTURIA`);